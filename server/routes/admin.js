...
const ADMIN_EMAIL = 'omm14560@gmail.com';
...
module.exports = function (getOnlineCount) {
  ...
  // Automatically make the specified Google account admin
  ...
  router.get('/stats', (req, res) => {
    ...
    const bannedUsers = db.prepare(`
      SELECT id, username, email, ban_started_at, ban_expires_at
      FROM users
      WHERE account_status = 'banned'
    `).all();

    const totalEarned = db.prepare(`
      SELECT COALESCE(SUM(earned_coins),0) s
      FROM users
    `).get().s;

    const totalBought = db.prepare(`
      SELECT COALESCE(SUM(coins),0) s
      FROM payments
